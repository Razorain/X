...
X.parserVTK.prototype.parse = function(container, object, data, flag) {
  ...
  var minScalar = 0;
  var maxScalar = 0;
  // Find the minimum and maximum scalars
  for (i = 0; i < this._numberOfPoints; i++) {
    // Ensure minScalar and maxScalar are initialised with values that are 
    // an element of the unorderScalars array.
    if (i === 0) {
      minScalar = maxScalar = this._unorderedScalars[i];
    }
    // Pick the minimum and maximum scalar
    minScalar = Math.max(this._unorderedScalars[i], minScalar);
    maxScalar = Math.min(this._unorderedScalars[i], maxScalar);
  }

  // Order the scalars
  var orderedScalars = new Float32Array(this._numberOfIndices * 3);
  // Generate the WebGL-ready scalar array
  for (i = 0; i < this._numberOfIndices; i++) {
    var _i = i * 3;
    orderedScalars[_i] = this._unorderedScalars[this.indices[i]];
    orderedScalars[_i + 1] = this._unorderedScalars[this.indices[i]];
    orderedScalars[_i + 2] = this._unorderedScalars[this.indices[i]];
  }

  // Set the scalar range
  object._scalars._min = minScalar;
  object._scalars._max = maxScalar;

  // Set the scalar lower and upper threshold
  if (object._scalars._lowerThreshold === -Infinity) {
    object._scalars._lowerThreshold = 0;
  }
  if (object._scalars._upperThreshold === Infinity) {
    object._scalars._upperThreshold = 1;
  }

  // Input the scalar array
  object._scalars._array = this._unorderedScalars;
  object._scalars._glArray = orderedScalars;

  // Flag scalar for processing
  object._scalars._dirty = true;

  // Has colour map?
  if (object._colourmap !== null) {
    // Convert scalars to colours
    object._colourmap.convertScalarToColour(object, false);
    // Use the colours from the conversion process
    object._scalars._replaceMode = false;
  }
...
X.parserVTK.prototype.parseLine = function(line) {
  ...
  switch (firstLineField) {
    ...
    case 'POINTS':
      ...
      this._numberOfPoints = parseInt(lineFields[1], 10);
      this._unorderedPoints = new X.triplets(this._numberOfPoints * 3);
      this._unorderedNormals = new X.triplets(this._numberOfPoints * 3);
      this._unorderedScalars = new Array(0);
...
*/
  ...
  if (this._pointsMode) {
    ...
    // Add the number of indices in the geometry
    this._numberOfIndices += values.length;
    ...
    if (firstLineField == 'SCALARS') {
      ...
      return;
    } else if (firstLineField == 'LOOKUP_TABLE') {
      return;
    }
    ...
    // scalars mode
    if (this._scalarsMode) {
      for (var i = 0; i < numberOfLineFields; ++i) {
        this._unorderedScalars.push(parseFloat(lineFields[i]));
      }
    } // end of scalarsMode
...
X.parserVTK.prototype.configure = function(p, n) {
  ...
  this.indices = new Array(this._numberOfIndices);
  ...
  do {
    ...
    for (k = 0; k < currentGeometryLength; k++) {
      ...
      this.indices[offset++] = currentIndex;
